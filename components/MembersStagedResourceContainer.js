...
function MembersStagedResourceContainer({room_id}) {
    ...
    useEffect(() => {
      const fetchData = async () => {
          ...
          const id = session.id
          const result = await axios.get(`${HOST_URL}/api/get_users_staged_res?room_id=${room_id}&user_id=${id}`)
...
export default MembersStagedResourceContainer